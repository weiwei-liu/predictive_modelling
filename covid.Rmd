...
plot(log(count.total)~day.total,pch=4,col="darkred",cex=.8) 
title("log(totalConfirmedCases) in Canada, Jan.22~Apr.15",font.main=3)
```

From this plot, it can be seen that in the first twenty days of the data, the number of cases changed very slowly, this could be a concern in the modelling in terms of including or excluding this periods.  If the first 20 days are included, the model can rarely be improved for future prediction since those data points are nearly 0 and barely change;  however if not the sample size is too small.  
...
count.total2<-diff(train1[which(train1['province']=='British Columbia'),]$ConfirmedCases)
day.total<-1:length(count.total2)
plot(count.total2, type = 'h',main="Daily Cases Increase in BC, Canada, Jan.22~Apr.15",
  xlab="Number of Days", col=c("royalblue"))
# plot(log(count.total+1)~day.total)    # use `+1` to avoid log(0)
```

This plot shows that in day 40-80, there are several aperiodic cycles, the daily increase is 0, which indicates the data was not updated daily, so this could cause some trouble in the modelling of this data.
...
day<-day[1:75]
day.test<-day[76:83]
plot(log(count)~day,pch=4,col="darkred",cex=.8)
title("Training Set: log(#Cases), Jan.22~Apr.6 Canada",font.main=3)
```
...
plot(day,log(count),pch=4,col="darkred",cex=.8)
quadline(covid.glm)
title("Fitting Training Set: GLM, Jan.22~Apr.6 Canada",font.main=3)
```
...
There are some concerns from this plot, the major one is that there are some outliers in the data strongly influenced the model from the `Cook's distance` plot, and the reponse data is not normally distributed from the `QQ` plot. In the `Scale-Location` plot, there is an upward trend which is concerning.  Generally the lower and upper bounds of datasets made the model inappropriate.  

### 1.4 Sub-conclusion 1 of GLM model:

Overall, this model is **NOT** an appropriate model mainly because the the reponse of the data after `log()` is not linear(the total number of cases in Canada is not strictly following anything log-like), so it is hard to use a linear model to fit the data. So nonlinear model could be a possible solution.
...
```{r}
plot(covid.gam)
title("Fitting Training Set: GAM, Jan.22~Apr.6 Canada",font.main=3)
```
...
Predict the test set(total number of cases) and evaluate the model: 
...
Second conclusion: based on this residual autocorrelation check, the model seems autocorrelated. So, `ARIMA` model could be another possible solution.  Let's have a try: 
...
In terms of AIC, clearly `ARIMA` model wins. The fitted ARIMA(0,1,2) model is : 

$\hat Y_t=20.7957+Y_{t-1}-1.3574e_{t-1}+0.7537e_{t-2}$

So given number of increase/cases, previous two error terms we can generally predict the following number within 2*sigma. 
